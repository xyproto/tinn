...
int main() {
  ...
  const Data data = build(DATADIR "semeion.data", nips, nops);
  // Train, baby, train.
  const Tinn::Tinn tinn = Tinn::xtbuild(nips, nhid, nops);
  for (int i = 0; i < 100; i++) {
    shuffle(data);
    float error = 0.0f;
  ...
  const Tinn::Tinn loaded = Tinn::xtload("saved.tinn");
  ...
  for (int i = 0; i < data.nops; i++) {
    printf("%f ", (double)tg[i]);
  }
  printf("\n");
  for (int i = 0; i < data.nops; i++) {
    printf("%f ", (double)pd[i]);
  }
  printf("\n");
